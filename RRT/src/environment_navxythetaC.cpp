/*
  This is the continuous version of an environment for navigation in x,y,theta 2D map.
  This environment is adapted from environment_navxyTHETA LAT.

  Since motion of a robot are not gauranteed to end in the center of a cell. Some
...
#define XYTHETA2INDEX(X,Y,THETA) (THETA + X*EnvNAVXYTHETACCfg.NumThetaDirs + \
...
bool EnvironmentNAVXYTHETAC::InitializeEnv(
	...
	expanded_heatmap_points.open("expanded_heatmap_points.txt");
	...
	if (fCfg == NULL) {
	 ...
	 if (sMotPrimFile != NULL) {
		...
		if (ReadMotionPrimitives(fMotPrim) == false) {
			...
			EnvNAVXYTHETACCfg.StartTheta = ContTheta2DiscNew(EnvNAVXYTHETACCfg.StartTheta_rad);
		...
		EnvNAVXYTHETACCfg.EndTheta = ContTheta2DiscNew(EnvNAVXYTHETACCfg.EndTheta_rad);
...
int EnvironmentNAVXYTHETAC::InitTree(int stateID) {
    ...
    // dimension is 3, x, y, theta
    kdtree = new kd_tree_t(3/*dim*/, treeNodes, nanoflann::KDTreeSingleIndexAdaptorParams(10));
...
// samples are inserted into the hash table
int EnvironmentNAVXYTHETAC::GetRandomSample() {
    sample_times++;
    ...
    double envWidth = EnvNAVXYTHETACCfg.EnvWidth_c * EnvNAVXYTHETACCfg.cellsize_m;
    double envHeight = EnvNAVXYTHETACCfg.EnvHeight_c * EnvNAVXYTHETACCfg.cellsize_m;

    // generate a state {x, y, theta}
    while(true) {
        float x, y, theta;

        // then sample in the goal region
        if (seed < 10) {
            x = bias.x_ + static_cast<float> (rand()) / (static_cast<float> (RAND_MAX / bias.rangex));
            y = bias.y_ + static_cast<float> (rand()) / (static_cast<float> (RAND_MAX / bias.rangey));
            theta = bias.theta_ + static_cast<float> (rand()) / (static_cast<float> (RAND_MAX / (bias.rangetheta)));       
            theta = normalizeAngle(theta);
        } else {
            x = static_cast<float> (rand()) / (static_cast<float> (RAND_MAX / envWidth));
            y = static_cast<float> (rand()) / (static_cast<float> (RAND_MAX / envHeight));
            theta = static_cast<float> (rand()) / (static_cast<float> (RAND_MAX /(M_PI * 2)));
        }


        if (nullptr == (HashEntry =(this->*GetHashEntry)(x,y,theta))) {

            HashEntry = (this->*CreateNewHashEntry)(x,y,theta);
            break;
            
            // the random example exist
        } else {
            continue;
        }
...
int EnvironmentNAVXYTHETAC::GetNN(int treeID, int queryStateID) {

    auto HashEntry = StateID2CoordTable.at(queryStateID);

    /* nanoflann */
    double query_pt[3] = {HashEntry->X, HashEntry->Y, HashEntry->Theta};
...
int EnvironmentNAVXYTHETAC::GetGoalStateID() {
    return EnvNAVXYTHETAC.goalstateid;
}

int EnvironmentNAVXYTHETAC::GetStartStateID() {
...
unsigned int EnvironmentNAVXYTHETAC::GETHASHBIN(
    ...
    unsigned int a;
    a = inthash(inthash(X1) + (inthash(X2) << 1) + (inthash(Theta) << 2)) & (HashTableSize - 1);
    return a;
}
...
void EnvironmentNAVXYTHETAC::InitializeEnvironment(){
	...
	if(maxsize <= SBPL_XYTHETAC_MAXSTATESFORLOOKUP){    // not using hash table
        ...
        Coord2StateIDHashTable = new std::vector<EnvNAVXYTHETACHashEntry_t*>[HashTableSize];
...
void EnvironmentNAVXYTHETAC::InitializeEnvConfig(std::vector<SBPL_xytheta_mprimitive>* motionprimitiveV){
	// additional to configuration file initialization of EnvNAVXYTHETACCfg if necessary
	// dXY dirs
...
void EnvironmentNAVXYTHETAC::DeprecatedPrecomputeActions(){
	...
	EnvNAVXYTHETACCfg.ActionsV = new EnvNAVXYTHETACAction_t* [EnvNAVXYTHETACCfg.NumThetaDirs];
	EnvNAVXYTHETACCfg.PredActionsV = new std::vector<EnvNAVXYTHETACAction_t*> [EnvNAVXYTHETACCfg.NumThetaDirs];
	std::vector<sbpl_2Dcell_t> footprint;
	// iterate over source angles
	for ( int tind = 0; tind < EnvNAVXYTHETACCfg.NumThetaDirs; ++tind){
		SBPL_PRINTF("processing angle %d\n", tind);
		EnvNAVXYTHETACCfg.ActionsV[tind] = new EnvNAVXYTHETACAction_t [EnvNAVXYTHETACCfg.actionwidth];
		...
		for(; aind<3; ++aind){
			...
			// -1,0,1
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta = (tind + aind -1)%EnvNAVXYTHETACCfg.NumThetaDirs;
			double angle =  DiscTheta2ContNew(EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta);
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].dX = (int)(cos(angle) + 0.5 * (cos(angle) > 0 ? 1 : -1));
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].dY = (int)(sin(angle) + 0.5 * (sin(angle) > 0 ? 1 : -1));
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].cost = (int)(ceil(NAVXYTHETAC_COSTMULT_MTOMM *
				EnvNAVXYTHETACCfg.cellsize_m / EnvNAVXYTHETACCfg.nominalvel_mpersecs *
				sqrt((double)(EnvNAVXYTHETACCfg.ActionsV[tind][aind].dX *
					EnvNAVXYTHETACCfg.ActionsV[tind][aind].dX + EnvNAVXYTHETACCfg.ActionsV[tind][aind].dY *
					EnvNAVXYTHETACCfg.ActionsV[tind][aind].dY))));

			// compute intersecting cells
			sbpl_xy_theta_pt_t pose;
			pose.x = DISCXY2CONT(EnvNAVXYTHETACCfg.ActionsV[tind][aind].dX, EnvNAVXYTHETACCfg.cellsize_m);
			pose.y = DISCXY2CONT(EnvNAVXYTHETACCfg.ActionsV[tind][aind].dY, EnvNAVXYTHETACCfg.cellsize_m);
			pose.theta = angle;
			...
			get_2d_footprint_cells(
							...
							pose,
							EnvNAVXYTHETACCfg.cellsize_m);
			RemoveSourceFootprint(sourcepose, &EnvNAVXYTHETACCfg.ActionsV[tind][aind].intersectingcellsV);

#if DEBUG
			SBPL_PRINTF("action tind=%d aind=%d: endtheta=%d (%f) dX=%d dY=%d cost=%d\n",
			tind, aind, EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta, angle,
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].dX, EnvNAVXYTHETACCfg.ActionsV[tind][aind].dY,
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].cost);
			 ...
			 EnvNAVXYTHETACCfg.PredActionsV[targettheta].push_back(&(EnvNAVXYTHETACCfg.ActionsV[tind][aind]));
		}
		 ...
		 if (EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta < 0) {
			 EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta += EnvNAVXYTHETACCfg.NumThetaDirs;
		 }
		 EnvNAVXYTHETACCfg.ActionsV[tind][aind].dX = 0;
		 EnvNAVXYTHETACCfg.ActionsV[tind][aind].dY = 0;
		 EnvNAVXYTHETACCfg.ActionsV[tind][aind].cost =
				 (int)(NAVXYTHETAC_COSTMULT_MTOMM * EnvNAVXYTHETACCfg.timetoturn45degsinplace_secs);

		 // compute intersecting cells
		 sbpl_xy_theta_pt_t pose;
		 pose.x = DISCXY2CONT(EnvNAVXYTHETACCfg.ActionsV[tind][aind].dX, EnvNAVXYTHETACCfg.cellsize_m);
		 pose.y = DISCXY2CONT(EnvNAVXYTHETACCfg.ActionsV[tind][aind].dY, EnvNAVXYTHETACCfg.cellsize_m);
		 pose.theta = DiscTheta2ContNew(EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta);
		 EnvNAVXYTHETACCfg.ActionsV[tind][aind].intermptV.clear();
		 EnvNAVXYTHETACCfg.ActionsV[tind][aind].intersectingcellsV.clear();
		 get_2d_footprint_cells(
				 EnvNAVXYTHETACCfg.FootprintPolygon,
		 ...
		 EnvNAVXYTHETACCfg.ActionsV[tind][aind].aind = aind;
		 EnvNAVXYTHETACCfg.ActionsV[tind][aind].starttheta = tind;
		 EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta = (tind + 1) % EnvNAVXYTHETACCfg.NumThetaDirs;
		 EnvNAVXYTHETACCfg.ActionsV[tind][aind].dX = 0;
		 EnvNAVXYTHETACCfg.ActionsV[tind][aind].dY = 0;
 ...
 #if DEBUG
		 SBPL_PRINTF("action tind=%d aind=%d: endtheta=%d (%f) dX=%d dY=%d cost=%d\n",
				 tind, aind, EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta, DiscTheta2ContNew(EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta),
					 EnvNAVXYTHETACCfg.ActionsV[tind][aind].dX, EnvNAVXYTHETACCfg.ActionsV[tind][aind].dY,
					 EnvNAVXYTHETACCfg.ActionsV[tind][aind].cost);
 #endif
		 ...
		 if (targettheta < 0) targettheta = targettheta + EnvNAVXYTHETACCfg.NumThetaDirs;
		 EnvNAVXYTHETACCfg.PredActionsV[targettheta].push_back(&(EnvNAVXYTHETACCfg.ActionsV[tind][aind]));
	 }
	...
	const std::vector<sbpl_2Dpt_t>& FootprintPolygon)
{
	...
	get_2d_footprint_cells(FootprintPolygon, &sourcefootprint, sourcepose, EnvNAVXYTHETACCfg.cellsize_m);
...
void EnvironmentNAVXYTHETAC::RemoveSourceFootprint(
	...
	std::vector<sbpl_2Dcell_t>* footprint)
{
	RemoveSourceFootprint(sourcepose, footprint, EnvNAVXYTHETACCfg.FootprintPolygon);
}
	...
	std::vector<SBPL_xytheta_mprimitive>* motionprimitiveV)
{
	...
	EnvNAVXYTHETACCfg.actionwidth = ((int)motionprimitiveV->size()) / EnvNAVXYTHETACCfg.NumThetaDirs;

	// iterate over source angles
	int maxnumofactions = 0;
	for (int tind = 0; tind < EnvNAVXYTHETACCfg.NumThetaDirs; tind++) {
		SBPL_PRINTF("pre-computing for angle %d out of %d angles\n", tind, EnvNAVXYTHETACCfg.NumThetaDirs);
		...
		for (int mind = 0; mind < (int)motionprimitiveV->size(); mind++) {
			...
			// compute dislocation
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta = motionprimitiveV->at(mind).endcell.theta;
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].dX = motionprimitiveV->at(mind).endcell.x;
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].dY = motionprimitiveV->at(mind).endcell.y;
			...
			for (int pind = 0; pind < (int)motionprimitiveV->at(mind).intermptV.size(); pind++) {
				...
				EnvNAVXYTHETACCfg.ActionsV[tind][aind].intermptV.push_back(intermpt);
				...
				intermediate2dCell.x = CONTXY2DISC(pose.x, EnvNAVXYTHETACCfg.cellsize_m);
				intermediate2dCell.y = CONTXY2DISC(pose.y, EnvNAVXYTHETACCfg.cellsize_m);

				// add unique cells to the list
				if (EnvNAVXYTHETACCfg.ActionsV[tind][aind].interm3DcellsV.size() == 0 ||
					intermediate2dCell.x != previnterm3Dcell.x ||
					intermediate2dCell.y != previnterm3Dcell.y)
				{
					EnvNAVXYTHETACCfg.ActionsV[tind][aind].interm3DcellsV.push_back(intermediate2dCell);
				}
			...
			for (unsigned int i = 1; i < EnvNAVXYTHETACCfg.ActionsV[tind][aind].intermptV.size(); i++) {
				double x0 = EnvNAVXYTHETACCfg.ActionsV[tind][aind].intermptV[i - 1].x;
				double y0 = EnvNAVXYTHETACCfg.ActionsV[tind][aind].intermptV[i - 1].y;
				double x1 = EnvNAVXYTHETACCfg.ActionsV[tind][aind].intermptV[i].x;
				double y1 = EnvNAVXYTHETACCfg.ActionsV[tind][aind].intermptV[i].y;
				double dx = x1 - x0;
				double dy = y1 - y0;
				linear_distance += sqrt(dx * dx + dy * dy);
			}
			double linear_time = linear_distance / EnvNAVXYTHETACCfg.nominalvel_mpersecs;
			double angular_distance;
			angular_distance = fabs(computeMinUnsignedAngleDiff(
					DiscTheta2ContNew(EnvNAVXYTHETACCfg.ActionsV[tind][aind].endtheta),
					DiscTheta2ContNew(EnvNAVXYTHETACCfg.ActionsV[tind][aind].starttheta)));
			...
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].cost =
					(int)(ceil(NAVXYTHETAC_COSTMULT_MTOMM * std::max(linear_time, angular_time)));
			// use any additional cost multiplier
			EnvNAVXYTHETACCfg.ActionsV[tind][aind].cost *= motionprimitiveV->at(mind).additionalactioncostmult;
			...
			SBPL_FPRINTF(fDeb,
						 ...
						 EnvNAVXYTHETACCfg.ActionsV[tind][aind].intermptV[0].theta * 180 / PI_CONST,
						 EnvNAVXYTHETACCfg.ActionsV[tind][aind].intermptV[EnvNAVXYTHETACCfg.ActionsV[tind][aind].intermptV.size() - 1].theta * 180 / PI_CONST, EnvNAVXYTHETACCfg.ActionsV[tind][aind].cost,
						 ...
						 (int)EnvNAVXYTHETACCfg.ActionsV[tind][aind].interm3DcellsV.size(),
						 (int)EnvNAVXYTHETACCfg.ActionsV[tind][aind].intersectingcellsV.size());
#endif
	...
	if (motionprimitiveV->size() != (size_t)(EnvNAVXYTHETACCfg.NumThetaDirs * maxnumofactions)) {
		std::stringstream ss;
...
void EnvironmentNAVXYTHETAC::SetConfiguration(
	...
	EnvNAVXYTHETACCfg.timetoturn45degsinplace_secs = timetoturn45degsinplace_secs;
	...
	for(int x = 0; x < EnvNAVXYTHETACCfg.EnvWidth_c; ++x){
		EnvNAVXYTHETACCfg.Grid2D[x] = new unsigned char[EnvNAVXYTHETACCfg.EnvHeight_c];
	}
...
void EnvironmentNAVXYTHETAC::ReadConfiguration(FILE* fCfg){
	...
	if (strcmp(sTemp1, sTemp) != 0) {
		// optional NumThetaDirs not available; default is NAVXYTHETAC_THETADIRS (16)
   ...
   // EnvNAVXYTHETACCfg.StartX_c = CONTXY2DISC(atof(sTemp), EnvNAVXYTHETACCfg.cellsize_m);
	...
	// EnvNAVXYTHETACCfg.StartY_c = CONTXY2DISC(atof(sTemp), EnvNAVXYTHETACCfg.cellsize_m);
	...
	// EnvNAVXYTHETACCfg.EndX_c = CONTXY2DISC(atof(sTemp), EnvNAVXYTHETACCfg.cellsize_m);
	if (fscanf(fCfg, "%s", sTemp) != 1) {
		throw SBPL_Exception("ERROR: ran out of env file early");
	}

	EnvNAVXYTHETACCfg.EndY_c = atof(sTemp);
	// EnvNAVXYTHETACCfg.EndY_c = CONTXY2DISC(atof(sTemp), EnvNAVXYTHETACCfg.cellsize_m);
	if (fscanf(fCfg, "%s", sTemp) != 1) {
		throw SBPL_Exception("ERROR: ran out of env file early");
	}
...
int EnvironmentNAVXYTHETAC::SetGoal(double x_m, double y_m, double theta_rad){
	...
	if(!IsWithinMapCell(x, y)){
		SBPL_ERROR("ERROR: tring to set goal cell %d %d that is outside the map\n", x, y);
		return -1;
	}
	...
	if( EnvNAVXYTHETAC.goalstateid != OutHashEntry->stateID){
       ...
       bias.rangex = bias.pct * EnvNAVXYTHETACCfg.cellsize_m * EnvNAVXYTHETACCfg.EnvWidth_c; 
       bias.rangey = bias.pct * EnvNAVXYTHETACCfg.cellsize_m * EnvNAVXYTHETACCfg.EnvHeight_c; 
       bias.rangetheta = bias.pct * M_PI * 2.0;
       bias.x_ = EnvNAVXYTHETACCfg.EndX_c - bias.rangex/2.0;
       bias.y_ = EnvNAVXYTHETACCfg.EndY_c - bias.rangey/2.0;
       bias.theta_ = EnvNAVXYTHETACCfg.EndTheta - bias.rangetheta/2.0;

       output << OutHashEntry->X << " " 
           << OutHashEntry->Y << " "
           << OutHashEntry->Theta << std::endl;
...
int EnvironmentNAVXYTHETAC::ContTheta2DiscFromSet(double theta) const {
	...
	if( EnvNAVXYTHETACCfg.NumThetaDirs >= (int) EnvNAVXYTHETACCfg.ThetaDirs.size()){
...
bool EnvironmentNAVXYTHETAC::IsWithinMapCell(int X,int Y){
	return (X >= 0 && X< EnvNAVXYTHETACCfg.EnvWidth_c &&
			Y >= 0 && Y< EnvNAVXYTHETACCfg.EnvHeight_c);
}


// now continuous pose are passed to this function directly
bool EnvironmentNAVXYTHETAC::IsValidConfiguration(double X, double Y, double Theta){
	...
	for(int find=0; find<(int)footprint.size();find++){
		...
		if( x<0 || x>= EnvNAVXYTHETACCfg.EnvWidth_c ||
			y<0 || y>= EnvNAVXYTHETACCfg.EnvHeight_c ||
...
void EnvironmentNAVXYTHETAC::SetAllActionsandAllOutcomes(CMDPSTATE* state){
	...
	for( int aind = 0; aind < EnvNAVXYTHETACCfg.actionwidth; ++aind){
		EnvNAVXYTHETACAction_t* nav3daction = &EnvNAVXYTHETACCfg.ActionsV[(unsigned int)HashEntry->Theta][aind];
		double newX = HashEntry->X + nav3daction->dX * EnvNAVXYTHETACCfg.cellsize_m;
		double newY = HashEntry->Y + nav3daction->dY * EnvNAVXYTHETACCfg.cellsize_m;
		...
		if (!IsValidCell(coverX,coverY)){ continue;}
...
void EnvironmentNAVXYTHETAC::GetCoordFromState(
	int stateID, double& x, double& y, double& theta) const{
...
int EnvironmentNAVXYTHETAC::GetStateFromCoord(double x, double y, double theta){
	EnvNAVXYTHETACHashEntry_t* OutHashEntry;
...
int EnvironmentNAVXYTHETAC::GetStartHeuristic(int stateID){
   ...
   if( stateID >= (int)StateID2CoordTable.size()){
		throw SBPL_Exception("ERROR in EnvNAVXYTHETAC... function: stateID illegal");
   }
#endif
   ...
   int hEuclid = (int)( NAVXYTHETAC_COSTMULT_MTOMM *
		EuclideanDistance_m(EnvNAVXYTHETACCfg.StartX_c, EnvNAVXYTHETACCfg.StartY_c, HashEntry->X, HashEntry->Y));

   // define this function if it is used in the planner (heuristic backward search would use it)
   return (int)(((double)__max(h2D, hEuclid))/EnvNAVXYTHETACCfg.nominalvel_mpersecs);
}
...
double EnvironmentNAVXYTHETAC::EuclideanDistance_m(double X1, double Y1, double X2, double Y2){
...
void EnvironmentNAVXYTHETAC::ComputeReplanningData()
{
	...
	for (int tind = 0; tind < EnvNAVXYTHETACCfg.NumThetaDirs; tind++) {
		...
		for (int aind = 0; aind < EnvNAVXYTHETACCfg.actionwidth; aind++) {
			...
			ComputeReplanningDataforAction(&EnvNAVXYTHETACCfg.ActionsV[tind][aind]);
		}
	}
}
	...
	EnvNAVXYTHETACAction_t* action)
{
	...
	for (int i = 0; i < (int)action->intersectingcellsV.size(); i++) {
		...
		if (bUseNonUniformAngles) {
			...
			endcell3d.theta = NORMALIZEDISCTHETA(action->endtheta, EnvNAVXYTHETACCfg.NumThetaDirs);
		}
...
bool EnvironmentNAVXYTHETAC::ReadMotionPrimitives(FILE* fMotPrims){
  ...
  //read in the resolution
	...
	if (bUseNonUniformAngles) {
		...
		EnvNAVXYTHETACCfg.ThetaDirs.push_back(2.0 * M_PI); // Add 2 PI at end for overlap
	}
	...
	for (int i = 0; i < totalNumofActions; i++) {
		...
		if (!EnvironmentNAVXYTHETAC::ReadinMotionPrimitive(&motprim, fMotPrims)) {
#if DEBUG
...
bool EnvironmentNAVXYTHETAC::ReadinCell(
...
bool EnvironmentNAVXYTHETAC::ReadinPose(
	sbpl_xy_theta_pt_t* pose,
	FILE* fIn){
	char sTemp[60];
...
EnvNAVXYTHETACHashEntry_t*
EnvironmentNAVXYTHETAC::GetHashEntry_hash(double X, double Y, double Theta){
	// returns a hash bin
	...
	std::vector<EnvNAVXYTHETACHashEntry_t*>* binV = &Coord2StateIDHashTable[binid];
...
EnvNAVXYTHETACHashEntry_t*
EnvironmentNAVXYTHETAC::CreateNewHashEntry_hash(double X, double Y, double Theta){
	int i;
...
EnvNAVXYTHETACHashEntry_t*
EnvironmentNAVXYTHETAC::GetHashEntry_lookup(double X, double Y, double Theta){
	if(X < 0 || X >= EnvNAVXYTHETACCfg.EnvWidth_c ||
	   Y < 0 || Y >= EnvNAVXYTHETACCfg.EnvHeight_c ||
	   Theta < 0 || Theta >= EnvNAVXYTHETACCfg.NumThetaDirs){
	   return NULL;
	}
...
EnvNAVXYTHETACHashEntry_t*
EnvironmentNAVXYTHETAC::CreateNewHashEntry_lookup(double X, double Y, double Theta){
	int i;
#if TIME_DEBUG
	clock_t  currenttime = clock();
#endif
...
// 		throw SBPL_Exception("ERROR: createing hash entry for non-NULL hashentry");
// 	}
// #endif
...
int EnvironmentNAVXYTHETAC::GetActionCost(
	double SourceX, double SourceY, double SourceTheta,
	EnvNAVXYTHETACAction_t* action){
	...
	// collision check-go over bounding box (minput and maxpt) to test validity and skip
	...
	for( i = 0; i< (int) action->interm3DcellsV.size(); ++i){
		...
		interm3Dcell.x = CONTXY2DISC( (interm3Dcell.x *cellsize) + SourceX, cellsize);

		interm3Dcell.y = CONTXY2DISC( (interm3Dcell.y *cellsize) + SourceY, cellsize);

		// out of map
		if(interm3Dcell.x < 0 || interm3Dcell.x >= EnvNAVXYTHETACCfg.EnvWidth_c ||
		   interm3Dcell.y < 0 || interm3Dcell.y >= EnvNAVXYTHETACCfg.EnvHeight_c){
		   return INFINITECOST;
		}

	   maxcellcost = __max(maxcellcost, EnvNAVXYTHETACCfg.Grid2D[interm3Dcell.x][interm3Dcell.y]);
	   ...
	   if( EnvNAVXYTHETACCfg.FootprintPolygon.size() > 1 &&
		  (int) maxcellcost >= EnvNAVXYTHETACCfg.cost_possibly_circumscribed_thresh){
			checks++;
	   ...
	   maxcellcost = __max(maxcellcost, EnvNAVXYTHETACCfg.Grid2D[intSourceX][intSourceY]);
	   int currentmaxcost = (int)__max(
			   maxcellcost,
			   EnvNAVXYTHETACCfg.Grid2D[intendX][intendY]);
...
double EnvironmentNAVXYTHETAC::GetDistance(int stateID1, int stateID2) {
    ...
    double result = diff0*diff0 + diff1*diff1;

    result +=  1.0 - std::cos(HashEntry1->Theta - HashEntry2->Theta) ;
...
void EnvironmentNAVXYTHETAC::GetPreds(
	int TargetStateID,
...
void EnvironmentNAVXYTHETAC::PrintState(
	int stateID,
	bool bVerbose,
	FILE* fOut){
#if DEBUG
	if(stateID >= (int)StateID2CoordTable.size()){
		SBPL_ERROR("ERROR in EnvNAVXYTHETAC... function: stateID illegal (2)\n");
		throw SBPL_Exception();
	}
#endif
	...
	int discx = CONTXY2DISC(HashEntry->X, EnvNAVXYTHETACCfg.cellsize_m);  
	int discy = CONTXY2DISC(HashEntry->Y, EnvNAVXYTHETACCfg.cellsize_m);
	expanded_heatmap_points << discx << " " << discy << " " << 1.4 << "\n";

}   // PrintState

void EnvironmentNAVXYTHETAC::PrintStateExpanded(
	int stateID)
{
	EnvNAVXYTHETACHashEntry_t* HashEntry = StateID2CoordTable[stateID];
	int discx = CONTXY2DISC(HashEntry->X, EnvNAVXYTHETACCfg.cellsize_m);  
...
void EnvironmentNAVXYTHETAC::EnsureHeuristicsUpdated(bool bGoalHeuristics){
	if(bNeedtoRecomputeStartHeuristics && !bGoalHeuristics){
		grid2Dsearchfromstart->search(
				EnvNAVXYTHETACCfg.Grid2D,
				EnvNAVXYTHETACCfg.cost_inscribed_thresh,
				CONTXY2DISC(EnvNAVXYTHETACCfg.StartX_c,EnvNAVXYTHETACCfg.cellsize_m),
				CONTXY2DISC(EnvNAVXYTHETACCfg.StartY_c,EnvNAVXYTHETACCfg.cellsize_m),
...
void EnvironmentNAVXYTHETAC::PrintEnv_Config(FILE* fOut){
	// implement this if the planner needs to print out EnvNAVXYTHETAC.configuration
	throw SBPL_Exception("ERROR in EnvNAVXYTHETAC... function: PrintEnv_Config is undefined");
}
...
void EnvironmentNAVXYTHETAC::PrintHashTableHist(FILE* fOut){
	...
	SBPL_FPRINTF(fOut, "hash tabel histogram: 0:%d, <5:%d, <25:%d, <50:%d, <100:%d, <400:%d, >400:%d\n",s0,s1,s50,
...
void EnvironmentNAVXYTHETAC::SetGoalTolerance(double tol_x,double tol_y, double tol_theta) {
...
// meter meter rad
bool EnvironmentNAVXYTHETAC::IsWithinGoalRegion(double x, double y, double theta) {

	if( sqrt( (x - EnvNAVXYTHETACCfg.EndX_c) * ( x - EnvNAVXYTHETACCfg.EndX_c) +
	    	  (y - EnvNAVXYTHETACCfg.EndY_c) * ( y - EnvNAVXYTHETACCfg.EndY_c) ) < EnvNAVXYTHETACCfg.tol_x &&
	   fabs(theta - EnvNAVXYTHETACCfg.EndTheta) < EnvNAVXYTHETACCfg.tol_theta ) {
...
int Dubin2Pt(double q[3], double x, void* subpath) {
    std::vector<sbpl_xy_theta_pt_t>* container = static_cast<std::vector<sbpl_xy_theta_pt_t>*>(subpath);
    sbpl_xy_theta_pt_t intermpt(q[0], q[1], q[2]);
    container->push_back(intermpt);
    return 0;
}
