...
void EnvironmentNAVXYTHETAC::ReadConfiguration(FILE* fCfg) {
    ...
    // unallocate the distance map
    // if (EnvNAVXYTHETACCfg.DistanceRatioMap != NULL)
    // {
    //     for (int x = 0; x < EnvNAVXYTHETACCfg.EnvWidth_c; ++x)
    //     {
    //         delete[] EnvNAVXYTHETACCfg.DistanceRatioMap[x];
    //     }
    //     delete[] EnvNAVXYTHETACCfg.DistanceRatioMap;
    //     EnvNAVXYTHETACCfg.DistanceRatioMap = NULL;
    // }

    // allocate the distance map
    unsigned char **Grid2D = EnvNAVXYTHETACCfg.Grid2D;
    int width_x = EnvNAVXYTHETACCfg.EnvWidth_c;
    int height_y = EnvNAVXYTHETACCfg.EnvHeight_c;
    unsigned char obsthresh = EnvNAVXYTHETACCfg.obsthresh;

    EnvNAVXYTHETACCfg.DistanceRatioMap = new float *[width_x];
    float **disttoNonfree_incells = new float *[width_x];
    for (int x = 0; x < width_x; ++x)
    {
        EnvNAVXYTHETACCfg.DistanceRatioMap[x] = new float[height_y];
        disttoNonfree_incells[x] = new float[height_y];
    }
    
    computeDistancestoNonfreeAreas(Grid2D, width_x, height_y, obsthresh, EnvNAVXYTHETACCfg.DistanceRatioMap, (float **)disttoNonfree_incells);
    // normalizing
    for (int x = 0; x < width_x; ++x)
    {
        for (int y = 0; y < height_y; ++y)
        {
            EnvNAVXYTHETACCfg.DistanceRatioMap[x][y] = EnvNAVXYTHETACCfg.DistanceRatioMap[x][y] / sqrt((width_x * width_x) + (height_y * height_y));
        }
    }

    // unallocate 2nd distance map
    // if (disttoNonfree_incells != NULL)
    // {
    //     for (int x = 0; x < EnvNAVXYTHETACCfg.EnvWidth_c; ++x)
    //     {
    //         delete[] disttoNonfree_incells[x];
    //     }
...
// returns true if there exists a distance in distances that is less than thresh
bool EnvironmentNAVXYTHETAC::IsLessThan(double distances[], int size, double thresh)
{
...
void EnvironmentNAVXYTHETAC::GetNeighborCellIds(int hashentryx, int hashentryy, int radius, std::vector<int> &NeighborsIds)
{
    // int counties = 0;
    for (int i = -radius; i <= radius; ++i)
    {
        for (int j = -radius; j <= radius; ++j)
        {
            ...
            NeighborsIds.push_back((hashentryx + i) +
                                (hashentryy + j) *
                                    EnvNAVXYTHETACCfg.EnvWidth_c);
            // counties++;
        }
...
int EnvironmentNAVXYTHETAC::GetGoalHeuristic(int stateID)
{
...
#endif
    EnvNAVXYTHETACHashEntry_t *HashEntry = StateID2CoordTable[stateID]; // current state
    auto parent = StateID2CoordTable[HashEntry->parentStateID];
    int hashentryx = CONTXY2DISC(HashEntry->X, EnvNAVXYTHETACCfg.cellsize_m); // x-coord
    int hashentryy = CONTXY2DISC(HashEntry->Y, EnvNAVXYTHETACCfg.cellsize_m); // y-coord
    double dH = 1.00;

#if PENALTY
    const auto start = std::chrono::system_clock::now();
    double position = parent->valid_children_ratio; //* (0.5 + (std::rand() % 10)/10); // gamma(s_parent)
    
    
    
    /* How to get a heatmap of states expanded:
     * Use plot_expanded.m in matlab folder
     * Note: This requires DEBUG_MODE == 1 in sbpl/src/include/sbpl/config.h
     */

    double amp = EnvNAVXYTHETACCfg.min_stepsize;
    double T = 2.0; // range for heuristic inflation
    // stepsize: radius in x,y space * gamma(s_parent)
    double stepsize = EnvNAVXYTHETACCfg.cellsize_m * amp * position;

    // R (normalization value) * gamma(s_parent)
    double radius =
        stepsize *
        EnvNAVXYTHETACCfg.R_times; // largest Euclidean distance + Angular distance
    // R_times: radius in .sg file for normalization

    
    

    // Neighbors is the container which stores neighbor cell stateIDs
    std::vector<int> neighbors;
    int range = (int)(radius * 0.5 / EnvNAVXYTHETACCfg.cellsize_m + 0.5);

    std::vector<double> new_ratio_vector;
    new_ratio_vector.push_back(range);
    new_ratio_vector.push_back(stepsize);


    if (mode == SUBTREE_LABELER || mode == SUBTREE_HASH)
    {
        NewRatioCalc(hashentryx, hashentryy, stateID, depth, new_ratio_vector);
    }

    // Because we are in the center of a (radius * radius) square grid
    if (mode == ORIGINAL)
    {
        // range is the discrete value for R (continious)
        
        // first 0.5 is for normalization
        // second 0.5 is for rounding down
    ...
    double Edist = 100; // Euclidean distance
    double closest = 100;
    double Sdist = closest;

    if (mode == ORIGINAL)
    {
        // vector storing all the states in one cell
        std::vector<EnvNAVXYTHETACHashEntry_t *> *neighborcell;
        EnvNAVXYTHETACHashEntry_t *tempEntry;
        // Loops through all neighbor cells
        for (int i = 0; i < (int)neighbors.size(); ++i)
        {
            // NN Table that we keep updating
            neighborcell = &NNtable[neighbors[i]];

            // Loops through all states in a neighbor cell
            for (int ind = 0; ind < (int)neighborcell->size(); ++ind)
            {
                ...
                if (tempEntry->parentStateID !=
                    ...
                    tempEntry->stateID != HashEntry->parentStateID)
                { // not parent
                    ...
                    Edist = sqrt((tempEntry->X - HashEntry->X) *
                                    ...
                                    (tempEntry->Y - HashEntry->Y) *
                                        (tempEntry->Y - HashEntry->Y));
                    // Space distance (Angular distance + Edist) == Distance in x,y,theta space
                    // Converting Angular distance into a Euclidean distance to combine with Edist:
                    // stepsize / 2.0 * (1.0 - cos(tempEntry->Theta - HashEntry->Theta))
                    // Done in this trial-and-error way because there is no concrete way to do this
                    // conversion
                    Sdist = stepsize / 2.0 *
    ...
    if (mode == SUBTREE_LABELER || mode == SUBTREE_HASH)
    {
        // closest = new_ratio_vector[1];
        new_ratio_vector[1] = new_ratio_vector[1] < radius ? new_ratio_vector[1] : radius;
        closest = new_ratio_vector[1]*(1.0 + c_value - new_ratio_vector[0]); // equation 9 code
    }
    // Checking to make sure NN is inside the circle within the square
    // Efficiency trick - useful for higher dimensions
    

    // inflation to heuristic
    // dup(s) = 1.0 - closest / radius
    // dH in range [1.0, T + 1.0] so that inflation is always >= 1
    float dup = 1.0 - closest / radius;
    // dup = (dup > 1.0) ? 1.0 : dup;
    dH = T * dup + 1.0;
    // return dH;
#endif
    
    // 2D heuristic from SBPL library
    int h2D = grid2Dsearchfromgoal->getlowerboundoncostfromstart_inmm(
        hashentryx, hashentryy);
    // EnvNAVXYTHETACCfg.nominalvel_mpersecs -> normalization for motion primitive
...
int EnvironmentNAVXYTHETAC::GetActionCost(
	...
	for( i = 0; i< (int) action->interm3DcellsV.size(); ++i){
		...
		if(maxcellcost >= EnvNAVXYTHETACCfg.cost_inscribed_thresh) return INFINITECOST;
...
void EnvironmentNAVXYTHETAC::GetSuccsNoHash(double x, double y, double theta, std::vector<std::vector<double>> &pointsvect)
{
    int aind;
    for (aind = 0; aind < EnvNAVXYTHETACCfg.actionwidth; aind++) {
        EnvNAVXYTHETACAction_t* nav3daction = &EnvNAVXYTHETACCfg.ActionsV[ContTheta2DiscNew(theta)][aind];
         // for disc theta in the expanded points use the function ContTheta2DiscNew
        
        double newX =
            x + (nav3daction->dX * EnvNAVXYTHETACCfg.cellsize_m);
        double newY =
            y + (nav3daction->dY * EnvNAVXYTHETACCfg.cellsize_m);
        double newTheta = DiscTheta2ContNew(nav3daction->endtheta);
        ...
        vect.push_back(newX);
        vect.push_back(newY);
        vect.push_back(newTheta);
        pointsvect.push_back(vect);
    }
}

double EnvironmentNAVXYTHETAC::SubtreeOverlapNewRatio(double x1, double y1, double t1, double x2, double y2, double t2, double stepsize)
{
    // Messy code, could be made better with loops. doesn't account for expansions accross all depths for Subtree Labeler, only for depth 3.
    ...
    std::vector<std::vector<double>> p1d4vect;
    std::vector<std::vector<double>> p2d4vect;
    std::set<std::vector<double>> tree1;
    std::set<std::vector<double>> tree2;
    int num_Identical = 0;
    GetSuccsNoHash(x1, y1, t1, p1d1vect);
    GetSuccsNoHash(x2, y2, t2, p2d1vect);
    ...
    if (depth == 1)
    {
        ...
        for (auto &point : p1d1vect)
        {
            ...
            {
                double tmp_distance = stepsize / 2.0 *
                                    (1.0 - cos(point1[2] - point[2])) +
                                EuclideanDistance_m(point[0], point[1], point1[0], point1[1]);
                if(tmp_distance <= SubtreeOverlapIdRadius){
                    num_Identical++;
                    break;
                } 
            }
        ...
        double eta = ((double)(num_Identical))/(p1d1vect.size()); 
        return eta;
    }
    else if (depth == 2)
    {
        for (auto &point : p1d1vect)
        {
            GetSuccsNoHash(point[0], point[1], point[2], p1d2vect);
            // code for equal depths comparisons
            for (auto &point1 : p2d1vect)
            {
                double tmp_distance = stepsize / 2.0 *
        ...
        for (auto &point : p2d1vect)
        {
            GetSuccsNoHash(point[0], point[1], point[2], p2d2vect);
        }
        // depth 2
        for (auto &point : p1d2vect)
        {
            // code for equal depths comparisons
            for (auto &point1 : p2d2vect)
            {
                double tmp_distance = stepsize / 2.0 *
                                    (1.0 - cos(point1[2] - point[2])) +
                                EuclideanDistance_m(point[0], point[1], point1[0], point1[1]);
        ...
        double eta = ((double)(num_Identical))/(p1d1vect.size() + p1d2vect.size()); 
        return eta;
    }
    else if (depth == 4)
    {
        ...
        for (auto &point : p1d2vect)
        {
            GetSuccsNoHash(point[0], point[1], point[2], p1d3vect);
            // code for equal depths comparisons
            for (auto &point1 : p2d2vect)
            {
                double tmp_distance = stepsize / 2.0 *
        ...
        for (auto &point : p2d2vect)
        {
            GetSuccsNoHash(point[0], point[1], point[2], p2d3vect);
            // tree2.insert(point);
        }
        // depth 3
        for (auto &point : p1d3vect)
        {
            GetSuccsNoHash(point[0], point[1], point[2], p1d4vect);
            // code for equal depths comparisons
            for (auto &point1 : p2d3vect)
            {
                double tmp_distance = stepsize / 2.0 *
                                    (1.0 - cos(point1[2] - point[2])) +
                                EuclideanDistance_m(point[0], point[1], point1[0], point1[1]);
        ...
        for (auto &point : p2d3vect)
        {
            GetSuccsNoHash(point[0], point[1], point[2], p2d4vect);
            // tree2.insert(point);
        }
        for (auto &point : p1d4vect)
        {
            // code for equal depths comparisons
            for (auto &point1 : p2d4vect)
            {
                double tmp_distance = stepsize / 2.0 *
                                    (1.0 - cos(point1[2] - point[2])) +
                                EuclideanDistance_m(point[0], point[1], point1[0], point1[1]);
        ...
        // code for Eta multiplied by a factor
        double eta = ((double)(num_Identical))/(p1d1vect.size() + p1d2vect.size() + p1d3vect.size()+ p1d4vect.size()); 
        return eta;
    }
    ...
    // code for Eta multiplied by a factor
    double eta = ((double)(num_Identical))/(p1d1vect.size() + p1d2vect.size() + p1d3vect.size()); 
    if (mode == SUBTREE_LABELER)
    {
        labeler_expansions += p1d1vect.size() + p1d2vect.size();
    }
    return eta;
    
}

void EnvironmentNAVXYTHETAC::NewRatioCalc(double x, double y, int stateID, bool one_state, std::vector<double> &position_and_dist){
    double range = position_and_dist[0];
    double stepsize = position_and_dist[1];
    position_and_dist.clear();
    
    std::vector<EnvNAVXYTHETACHashEntry_t *> *neighborcell;
    std::vector<int> NeighborsIDs;
    EnvNAVXYTHETACHashEntry_t *HashEntry = StateID2CoordTable[stateID];
    double theta = HashEntry->Theta;
    EnvNAVXYTHETACHashEntry_t *tempEntry;

    GetNeighborCellIds(x, y, range, NeighborsIDs);
    x = HashEntry->X;
    y = HashEntry->Y;
    int counter = 0;
    double label_sum = 0;
    ...
    double label = 0;
    double dist = 100.0;
    double finalx, finaly, finalt;
    for(int i = 0; i < NeighborsIDs.size(); i++)
    {
        // NN Table that we keep updating
        neighborcell = &NNtable[NeighborsIDs[i]];
        ...
        for (int ind = 0; ind < (int)neighborcell->size(); ++ind)
        {
            ...
            if (tempEntry->stateID != stateID)
            { // not self
                double nhashentryx = tempEntry->X; // x-coord
                double nhashentryy = tempEntry->Y; // y-coord
                double nhashentryt = tempEntry->Theta;
                // code for maximum duplicity in epsilon ball and data generation
                double curr_label;
                if (mode == SUBTREE_LABELER) curr_label = SubtreeOverlapNewRatio(x, y, theta, nhashentryx, nhashentryy, nhashentryt, stepsize);
                else
                {
                    double delta_x = x - nhashentryx;
                    double delta_y = y - nhashentryy;
                    delta_x = fabs(delta_x) < 0.001 ? 0 : delta_x;
                    delta_y = fabs(delta_y) < 0.001 ? 0 : delta_y;
                    delta_x*=40;
                    delta_y*=40;
                    curr_label = labeler_hash[3 - int(delta_x)][3 - int(delta_y)][ContTheta2DiscNew(theta)][ContTheta2DiscNew(nhashentryt)];
                    if (curr_label == -1) curr_label = SubtreeOverlapNewRatio(x, y, theta, nhashentryx, nhashentryy, nhashentryt, stepsize);
                }
                if (curr_label > label)
                {
                    finalx = nhashentryx;
                    ...
                    counter++;
                    label = curr_label;
                    dist = EuclideanDistance_m(HashEntry->X, HashEntry->Y, tempEntry->X, tempEntry->Y);
                    dist = stepsize / 2.0 *
                                (1.0 - cos(tempEntry->Theta - HashEntry->Theta)) +
                            dist;
                }
    ...
    position_and_dist.push_back(label);
    position_and_dist.push_back(dist);
    return;
}
