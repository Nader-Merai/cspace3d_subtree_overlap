...
struct EnvNAVXYTHETACHashEntry_t{
    int stateID;
    int parentStateID;
    double valid_children_ratio; // gamma(state)
    double X;
    double Y;
    double Theta;
    int iteration;
...
//----------------- part2 run --------------------------------------------
    ...
    */
    ...
    virtual void GetSuccsNoHash(double x, double y, double theta, std::vector<std::vector<double>> &pointsvect);
    // subtree overlap

    virtual void GetNeighborCellIds(int hashentryx,int hashentryy, int radius, std::vector<int> &NeighborsIds);

    // virtual int SubtreeOverlap(int x1, int y1, int t1, int x2, int y2, int t2);

    virtual double SubtreeOverlapNewRatio(double x1, double y1, double t1, double x2, double y2, double t2, double stepsize);

    virtual void NewRatioCalc(double x, double y, int stateID, bool one_state, std::vector<double> &position_and_dist);
...
//    EnvNAVXYTHETACHashEntry_t* (EnvironmentNAVXYTHETAC::*CreateNewHashEntry)(int X, int Y, int Theta);
    ...
    virtual bool IsLessThan(double distances[], int size, double thresh);
